...
def centrality(sent_tokens, cv, word_tokens_refined):
    ...
    l = Tf_Idf.shape[0]  # Actual number of sentences
    centrality_value = {}
    ...
    for i in range(l):
        ...
        for j in range(l):
            ...
            len_vec1 = 0
            len_vec2 = 0
            for k in range(Tf_Idf.shape[1]):
                dot_product += Tf_Idf[i][k] * Tf_Idf[j][k]
                len_vec1 += Tf_Idf[i][k] ** 2
                len_vec2 += Tf_Idf[j][k] ** 2
...
def get_data(text, text1, flag, word_limit=200):
    global corpus
    corpus = []  # Reset the corpus to avoid index mismatch
    ...
    n = list(n.values())
    c = centrality(sent_tokens, cv, word_tokens_refined)  # <-- fixed
    c = list(c.values())
